 ...
 */
export const filterEq = <T>(key: keyof T, value: T[keyof T]) =>
 ...
 * @returns Subset of the input stream emitting only items matching obj[key] !== value
 */
export const filterNeq = <T>(key: keyof T, value: T[keyof T]) =>
  filter((v) => v[key] !== value);
 ...
 */
export const mapMessage = map((v: string | LoggedMessage) =>
  typeof v !== 'string' ? v : createLoggedMessage(v)
);

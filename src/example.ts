...
import { HasEventName, LoggedEvent, PlayerJoinedEvent, PlayerLeftEvent } from './lib/types';
import {
    createLoggedEvent,
    createLoggedMessage,
    streamLoggedEvents,
...
export const startStreamsDemo = (showDebug?: boolean) => {
    const lines$ = streamLoggedLines(join(process.cwd(), 'sample.log'), {
        fromBeginning: true,
    });
    ...
    if (showDebug === true)
        messages$.subscribe((v) =>
            console.debug(`[${v?.timestamp}] ${v?.message}`)
        );
    if (showDebug === true)
        events$.subscribe((v) =>
            console.debug(
                `[${v?.timestamp}] {${v?.eventName || '...'}} -> ${v?.message}`
            )
        );
    ...
    playerJoined$.subscribe((v) =>
        console.log(`[${v.timestamp}] <-- ${v.playerName} joined`)
    );

    const playerLeft$ = streamLoggedEventsNamed(
        messages$,
        'playerLeft'
    ) as Observable<LoggedEvent<PlayerLeftEvent>>;

    playerLeft$.subscribe((v) =>
        console.log(`[${v.timestamp}] --> ${v.playerName} left`)
    );
    ...
    const mapToPresenceEvent = map(
        (v: LoggedEvent) =>
            v as LoggedEvent<PlayerJoinedEvent | PlayerLeftEvent>
    )

    const presence$ = events$.pipe(
        filterPresenceEvents,
        mapToPresenceEvent
    );

    presence$.subscribe(({ eventName, timestamp, playerName }) => {
        console.log(
            `[${timestamp}] ${eventName}(${playerName})`
        );
    });
};
